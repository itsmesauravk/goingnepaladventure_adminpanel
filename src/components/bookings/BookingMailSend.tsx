...
const BookingMailSend: React.FC<MailSendFormProps> = ({
  email,
  name,
  onChange,
  id,
  ...
  const handleSendEmail = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL_DEV}/booking/send-booking-mail/${id}`,
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        onChange(1)
        setLoading(false)
        toast.success(`Email sent successfully to ${name || recipient}`)
        // Reset form
        setRecipient("")
        setSubject("")
        setMessage("")
    ...
    <div className="w-[400px] mx-auto p-6 bg-white shadow-md rounded-lg">
      <form onSubmit={handleSendEmail}>
        ...
        <div className="text-gray-600 text-sm mt-2 italic">
          Note: Please ensure all fields are filled out correctly before
          sending.
          <br />
          If any problem occures, try reloading page.
        </div>
      </form>
    </div>
  )
...
export default BookingMailSend
